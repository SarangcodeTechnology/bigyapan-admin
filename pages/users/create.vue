...
export default {
  ...
  computed: {
    ...mapGetters("user", ["isCreating", "createdData"]),
    ...
    passwordErrors() {
      ...
      !this.$v.user.password.minLength && errors.push('Password must be minimum 8 characters.');
      return errors;
    },
    confirmPasswordErrors() {
      const errors = [];
      if (!this.$v.user.password_confirmation.$dirty) return errors;
      !this.$v.user.password_confirmation.required && errors.push('Confirm Password is required.');
      !this.$v.user.password_confirmation.sameAsPassword && errors.push('Password doesn\'t match');
      return errors;
    }
  },
  validations: {
